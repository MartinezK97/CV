<?php 
    class otro extends Model{
        ...
        function get($id){
            try{
                $query = $this->prepare('SELECT * FROM otro WHERE id = :id');
                $query->execute(['id'=>$id]);
        ...
        function getAll(){
            try{
                $query = $this->prepare('SELECT * FROM otro');
                $query->execute([]);
                if($query->rowCount() > 0 ):
        ...
        function delete($id){
            ...
            try{
                $query = $this->prepare('DELETE FROM otro WHERE id = :id');
                $query->execute(['id'=>$id]);
        ...
        function update($id){
            try{
                $query = $this->prepare('UPDATE otro SET(value = :value) WHERE id = :id');
